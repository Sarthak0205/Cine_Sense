from __future__ import annotations

import json
import os
import sys
from pathlib import Path


PROJECT_ROOT = Path(__file__).resolve().parents[1]
if str(PROJECT_ROOT) not in sys.path:
    sys.path.insert(0, str(PROJECT_ROOT))
os.chdir(PROJECT_ROOT)

from evaluation.benchmark import evaluate_model, relevance_scores_from_interactions, save_results
from evaluation.cinesense_single_seed_reranked import CineSenseSingleSeedMaxReranked
from evaluation.datasets import (
    ITEM_ID_COL,
    build_eval_users,
    build_positive_interactions,
    filter_users,
    load_anime_catalog,
    load_user_watches,
    split_user_interactions,
)


RESULTS_DIR = Path("evaluation/results")
WEIGHT_CONFIGS = {
    "A": (0.70, 0.20, 0.10),
    "B": (0.60, 0.25, 0.15),
    "C": (0.50, 0.35, 0.15),
}


def main() -> None:
    catalog = load_anime_catalog()
    user_watches = load_user_watches()
    positives = build_positive_interactions(
        user_watches,
        catalog_item_ids=catalog[ITEM_ID_COL].unique(),
    )
    filtered_users = filter_users(positives)
    split = split_user_interactions(filtered_users)
    eval_users = build_eval_users(split, use_validation=True)
    relevance_scores_by_user = relevance_scores_from_interactions(split.validation)

    results_by_config = {}
    model = CineSenseSingleSeedMaxReranked().fit(catalog)

    for label, weights in WEIGHT_CONFIGS.items():
        semantic_weight, keyword_weight, theme_weight = weights
        model.model_name = f"cinesense_single_seed_max_reranked_{label.lower()}"
        model.semantic_weight = semantic_weight
        model.keyword_weight = keyword_weight
        model.theme_weight = theme_weight

        results = evaluate_model(
            model,
            eval_users,
            use_validation=True,
            relevance_scores_by_user=relevance_scores_by_user,
            max_users=1000,
        )
        result_path = RESULTS_DIR / f"cinesense_single_seed_max_reranked_{label.lower()}_validation.json"
        save_results(
            results,
            result_path,
            split_seed=split.seed,
            train_ratio=split.train_ratio,
            val_ratio=split.val_ratio,
            test_ratio=split.test_ratio,
        )
        results_by_config[label] = results["metrics"]

    print(json.dumps(results_by_config, indent=2, sort_keys=True))
    print_comparison_table(results_by_config)


def print_comparison_table(results_by_config: dict[str, dict[str, float]]) -> None:
    rows = [
        ("Popularity", 0.0481, 0.0803, 0.0745, 0.4520),
        ("Single-Seed Max", 0.0445, 0.0876, 0.0392, 0.3910),
    ]
    for label in ["A", "B", "C"]:
        metrics = results_by_config[label]
        rows.append(
            (
                f"Reranked {label}",
                metrics["recall@10"],
                metrics["recall@20"],
                metrics["ndcg@10"],
                metrics["hit_rate@10"],
            )
        )

    print()
    print(f"{'Model':<18} {'Recall@10':>10} {'Recall@20':>10} {'NDCG@10':>10} {'HitRate@10':>11}")
    for model_name, recall_10, recall_20, ndcg_10, hit_rate_10 in rows:
        print(
            f"{model_name:<18} "
            f"{recall_10:>10.4f} "
            f"{recall_20:>10.4f} "
            f"{ndcg_10:>10.4f} "
            f"{hit_rate_10:>11.4f}"
        )


if __name__ == "__main__":
    main()
