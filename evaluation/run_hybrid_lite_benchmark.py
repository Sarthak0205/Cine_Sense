...
from cinesense.recommenders.base import CineSenseHybridLite
...
def main() -> None:
    ...
    results_by_config = {}
    model = CineSenseHybridLite().fit(catalog, split.train)

    for label, weights in WEIGHT_CONFIGS.items():
        semantic_weight, popularity_weight = weights
        model.model_name = f"cinesense_hybrid_lite_{label.lower()}"
        model.semantic_weight = semantic_weight
        model.popularity_weight = popularity_weight
        ...
        result_path = RESULTS_DIR / f"cinesense_hybrid_lite_{label.lower()}_validation.json"
        save_results(
            results,
            result_path,
